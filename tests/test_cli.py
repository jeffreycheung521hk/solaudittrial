...
class EnumerationCliTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_enumerate_cli_writes_epoch_aware_sanitized_diff_and_summary(self) -> None:
        ...
        self.assertEqual(row["verdict"], "UNCONFIRMED_OMISSION")
        self.assertFalse(row["confirmed"])
        self.assertFalse(row["conclusive"])
        self.assertFalse(row["evidence"]["direct_getblock_issued"])
        self.assertTrue(row["evidence"]["unexcluded_explanations"])
        self.assertNotIn("proving", row["evidence"]["reasoning"])
        ...
        self.assertTrue(
            any("confirmed provider data hole" in item
        ...
        self.assertEqual(summary["unconfirmed_omissions"], 1)
        by_provider = {item["provider"]: item for item in summary["providers"]}
        self.assertEqual(by_provider["provider-a"]["unconfirmed_omissions"], 1)
        self.assertEqual(by_provider["provider-b"]["unconfirmed_omissions"], 0)
